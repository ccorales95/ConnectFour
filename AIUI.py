...
while (user1Won + user2Won) == 0:
    ...
    print("AI Moving...")
    validPlay = False
    bestplay, bestscore = minmaxplay(myBoard, 3, 'x')
    #just accommodating an off-by-one error caused by our difference in convention
    play = bestplay + 1
    while not validPlay:
        while (play < 1) or (play > 7):
            print("The AI probably broke catastrophically. Whoops.")
